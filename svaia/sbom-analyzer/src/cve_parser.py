...
def format_single_cve(cve):
    """Formatea objeto CVE para una salida legible."""
    
    result = "\n" + "=" * 80
    result += "\n" + f"\033[1;31mCVE ID:\033[0m \033[1m{cve.id}\033[0m"
    result += "\n" + f"\033[1;34mEstado:\033[0m {getattr(cve, 'vulnStatus', 'Desconocido')}"
    result += "\n" + f"\033[1;34mPublicado:\033[0m {cve.published}"
    result += "\n" + f"\033[1;34mÚltima modificación:\033[0m {cve.lastModified}"
    ...
    result += "\n\033[1;33mDESCRIPCIÓN:\033[0m\n"
    result += textwrap.fill(es_desc if es_desc else en_desc, width=80)

    # CVSS v3.1
    if hasattr(cve, 'metrics') and getattr(cve.metrics, 'cvssMetricV31', None):
        cvss = cve.metrics.cvssMetricV31[0].cvssData
        result += "\n\033[1;33mCVSS v3.1:\033[0m\n"
        result += f"  \033[1;36mPuntuación:\033[0m {cvss.baseScore} ({cvss.baseSeverity})"
        result += f"  \033[1;36mVector:\033[0m {cvss.vectorString}"
        result += f"  \033[1;36mImpacto:\033[0m Confidencialidad: {cvss.confidentialityImpact}, Integridad: {cvss.integrityImpact}, Disponibilidad: {cvss.availabilityImpact}"
        ...
        for node in cve.configurations[0].nodes:
            for match in node.cpeMatch:
                if match.vulnerable:
                    version_info = f" (Versiones anteriores a {match.versionEndExcluding})" if hasattr(match,
                                                                                                       'versionEndExcluding') else ""
                    result += f"  - {match.criteria}{version_info}\n"
...
if __name__ == "__main__":
    ...
    formated = format_single_cve(results[0])
    print(formated)
    print(len(formated))
